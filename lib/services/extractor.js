...
// Extract text from a PDF buffer using unpdf (pdfjs-dist, serverless-safe)
// Returns { text, pageCount, error, warnings }
async function extractFromPDF(buffer) {
  try {
    // unpdf uses the latest pdfjs-dist with built-in XRef repair and
    // serverless-safe globals — more reliable than pdf-parse for malformed PDFs
    const uint8 = new Uint8Array(buffer);
    const { text: rawText, totalPages } = await unpdfExtract(uint8, {
      mergePages: true,
    });
    ...
    // Warn if PDF likely contains tables — pdfjs has no table awareness
    ...
    return { text, pageCount: totalPages || null, error: null, warnings };
  } catch (error) {
    return {
      text: null,
      pageCount: null,
