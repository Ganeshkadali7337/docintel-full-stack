...
// Download a file from Cloudinary by its stored URL and return a Buffer
// Uses Node's native https module to bypass Next.js's patched fetch,
// which can silently fail in background (fire-and-forget) tasks
export function downloadFile(cloudinaryUrl) {
  return new Promise((resolve, reject) => {
    const https = require('https')
    https.get(cloudinaryUrl, (response) => {
      if (response.statusCode !== 200) {
        reject(new Error(`Cloudinary download failed: ${response.statusCode} ${response.statusMessage}`))
        return
      }
      const chunks = []
