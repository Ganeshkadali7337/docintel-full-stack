...
// Find an existing READY document with the same file hash for this user
// Returns null if no duplicate exists
export async function getDocumentByHash(fileHash, userId) {
  return await prisma.document.findFirst({
    where: { fileHash, userId, status: 'READY' },
  })
}
